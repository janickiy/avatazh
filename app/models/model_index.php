...
class Model_index extends Model
{
    ...
    {
        $query = "SELECT * FROM " . core::database()->getTableName('shops') . " WHERE status=1 AND city=" . $city;
        $result = core::database()->querySQL($query);
        return core::database()->getColumnArray($result);
    }
     ...
     * @return mixed
     */
    public function deleteCars($activate)
    {
        $temp = [];
        ...
        return core::database()->delete(core::database()->getTableName('price'), "id IN (" . implode(",", $temp) . ")");
    }
}